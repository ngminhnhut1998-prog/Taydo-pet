...
export function PetAppointmentsCard() {
  const pets = useLiveQuery(() => db.pets.limit(5).toArray(), []);

  if (pets === undefined) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Thú cưng gần đây</CardTitle>
      ...
      <CardContent>
        {pets.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4 py-10 text-center">
            ...
            <p className="text-muted-foreground">Hãy bắt đầu bằng cách thêm khách hàng và thú cưng của họ.</p>
          </div>
        )}
      </CardContent>
    </Card>
